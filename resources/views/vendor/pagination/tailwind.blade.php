@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between mt-16">
        ...
        <div class="flex gap-1">
            ...
            @else
                <a wire:navigate href="{{ $paginator->previousPageUrl() }}" rel="prev" class="flex items-center justify-center w-8 h-8 text-blue-400 transition-colors bg-gray-100 rounded-sm hover:bg-gray-200/50 hover:text-blue-500">
                    ←
                </a>
            @endif
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <a wire:navigate href="{{ $url }}" class="flex items-center justify-center w-8 h-8 text-blue-400 transition-colors bg-gray-100 rounded-sm hover:bg-gray-200/50 hover:text-blue-500">
                                {{ $page }}
                            </a>
                        @endif
                    @endforeach
            ...
            @if ($paginator->hasMorePages())
                <a wire:navigate href="{{ $paginator->nextPageUrl() }}" rel="next" class="flex items-center justify-center w-8 h-8 text-blue-400 transition-colors bg-gray-100 rounded-sm hover:bg-gray-200/50 hover:text-blue-500">
                    →
                </a>
            @else
