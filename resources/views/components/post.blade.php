        ...
        <div>
            <div class="flex items-center gap-2 text-sm">
                ...
                <div class="line-clamp-1">
                    <a href="{{ route('consulting.cto') }}" target="_blank" rel="noopener noreferrer" class="font-medium" @click="window.fathom?.trackGoal('LNRXVF3B', 0)">{{ $post->user->name }}</a> — <span class="sr-only sm:not-sr-only">Updated on</span> <time datetime="{{ $post->modified_at?->toDateTimeString() }}">{{ $post->modified_at?->isoFormat('ll') }}</time>
                </div>
            </div>
